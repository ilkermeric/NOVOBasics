...
print("The total load (hits / incident photon) is: " + str(TotalLoad))
print("The highest load (hits / incident photon) per segment is: " + str(HighestLoad))
...
plt.title( str(IncidentParticleEnergy) + " MeV photons with segmentation")
plt.xlabel("y [cm]")
plt.ylabel("z [cm]")
#plt.yscale("log")
#plt.colorbar().set_label("Total load per incident neutron", rotation=270, labelpad=20)
cb = plt.colorbar(ticks=v1)
cb.set_label("Load per incident photon", rotation=270, labelpad=20)
...
f1.write('Total load (hits / incident photon) is: ' + str(TotalLoad) + '/n')
f1.write('Max. load (hits / incident photon / segment) is: ' + str(HighestLoad) + '/n')
f1.write('Min. load (hits / incident photon / segment) is: ' + str(SmallestLoad) + '/n')
